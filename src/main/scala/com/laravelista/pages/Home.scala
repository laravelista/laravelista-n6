...
val ctaButtonClasses =
  "font-medium bg-black dark:bg-white text-zinc-100 dark:text-zinc-900 rounded-sm inline-block hover:bg-zinc-700 dark:hover:bg-zinc-200 duration-300"
...
val bookSection =
  div(
    ...
    div(
      ...
      figure(
        cls := "box box-lg relative w-full p-12 bg-zinc-100 dark:bg-zinc-900 text-zinc-700 dark:text-zinc-100 mb-10 mt-10 rounded-lg hover:before:opacity-100 hover:scale-[1.01] duration-300",
...
def bwCard(
  ...
  div(
    cls := "w-full bg-zinc-100 dark:bg-zinc-900 rounded-lg box box-sm relative px-6 py-4 z-10 hover:before:opacity-100 duration-300 hover:scale-[1.01]",
    link match
      case Some(url) =>
        a(
          href := url.toString,
...
def itemSection(title: String, items: List[Item]) =
  div(
    ...
    div(
      ...
      for item <- items
      yield div(
        cls := "box box-sm relative p-12 bg-zinc-100 dark:bg-zinc-900 rounded-lg hover:scale-[1.01] duration-300 hover:before:opacity-100",
        div(
          cls := "flex",
          item.tag.getSvg,
          div(
